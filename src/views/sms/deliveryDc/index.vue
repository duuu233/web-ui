<template>
  <div class="app-container">
    <div class="map-box">
      <div class="list">
        <!-- <el-radio-group v-model="isCircle" @change="handleCircle">
          <el-radio :label="1">开启画圆</el-radio>
          <el-radio :label="0">关闭画圆</el-radio>
        </el-radio-group> -->
        <div class="list-box">
          <h4>区域列表</h4>
          <div
          ...
          >
            <div class="text">{{ item.dcName }}</div>
            <div class="handle">
              <i @click.stop="edit(index)" v-if="item.verify">编辑</i>
              <i @click.stop="look(index)" style="padding: 0 3px">查看</i>
              <el-tooltip
                :content="item.verify === 1 ? '启用中' : '已禁用'"
                placement="top"
              >
                <el-switch
                  v-permission="['post_deliveryDc_setDeliveryDcVerify']"
    ...
    >
      <el-form
        ...
        label-width="120px"
      >
        <el-form-item label="区域名称" prop="dcName">
          <el-input
            v-model="form.dcName"
            style="width: 350px"
            :disabled="isLook"
        ...
        <el-form-item label="区域坐标" prop="address" v-if="isLook">
          <el-input
        ...
        <el-form-item label="配送区域联系人" prop="contact">
          <el-input
        ...
        <el-form-item label="配送区域联系电话" prop="contactPhone">
          <el-input
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button
          type="primary"
          v-permission="['post_deliveryDc_setDeliveryDc']"
...
export default {
  ...
  mounted() {
    this.cityName = getCookie('cityName')
    AMapLoader.load({
      key: '32602678c4b32a84d40b1e5b78059f21', // 申请好的Web端开发者Key，首次调用 load 时必填 ed3d71e537761b02f1d9e5db5603db3c
  ...
  methods: {
    edit(index) {
      ...
      // 先清空所有圆，再单独渲染编辑的这个圆，防止2个圆重叠时出现的层级问题。
      let removeList = []
      this.circleObjList.forEach((v, i) => {
        if (i !== index) {
          removeList.push(v)
        }
      })
      this.map.remove(this.textList)
      this.map.remove(removeList)
      this.rightClickCircleObj = this.circleObjList[index]
    ...
    handleStatusChange(status, row) {
      let text = `确定要${row.verify === 1 ? '启用' : '禁用'}该区域吗?`
    ...
    clickList(index) {
      let key = -1
      if (this.circleObjList.length) {
        this.circleObjList.forEach((v, i) => {
          if (
            v._opts.center[0] * 1 === this.leftTextList[index].longitude * 1 &&
            v._opts.center[1] * 1 === this.leftTextList[index].latitude * 1
          ) {
            key = i
          }
        })
        if (key !== -1) {
          this.map.setFitView([this.circleObjList[key]])
        }
      }
    },
    addCircle(isDefault) {
    ...
    cancelCircle() {
      ...
      if (this.isEdit) {
        this.getList()
        // // 取消编辑 ，，先把以前的都删除了，再依次添加圆 和 标题
        // this.map.remove(this.rightClickCircleObj);
        // // 还原生成原来的圆
        // var circle = new AMap.Circle({
        //   center: [
        //     this.leftTextList[this.editIndex].longitude * 1,
        //     this.leftTextList[this.editIndex].latitude * 1,
        //   ],
        //   radius: this.leftTextList[this.editIndex].radius, //半径
        //   borderWeight: 3,
        //   strokeColor: "#FF33FF",
        //   strokeOpacity: 1,
        //   strokeWeight: 6,
        ...
        // });
        // this.rightClickCircleObj = circle;
        // // this.circleObjEditor = new AMap.CircleEditor(this.map, circle);
        // this.circleObjEditor.close();
        // //覆盖物--圆--绑定鼠标右击事件——弹出右键菜单
        // circle.on("rightclick", (e) => {
        //   // if (
        ...
        //   //   isEditorStatus = false;
        //   // }
        //   this.circleMenuFn(this.AMap, false);
        //   this.editLnglatList[0] = e.lnglat.lng;
        //   this.editLnglatList[1] = e.lnglat.lat;
        //   this.circleMenu.open(this.map, e.lnglat);
        // });
        ...
        //   text: this.leftTextList[this.editIndex].dcName,
        ...
        //   position: [
        //     this.leftTextList[this.editIndex].longitude,
        //     this.leftTextList[this.editIndex].latitude,
        //   ],
        // });
      ...
      this.$refs.refForm.validate(valid => {
        if (valid) {
          ...
          if (this.isEdit) {
            ...
            if (!this.form.dcName) {
              form.dcName = this.form.address
            }
            form.radius = this.circleObj._opts.radius
            let longitude = this.circleObj._opts.center.lng
              ? this.circleObj._opts.center.lng
              : this.circleObj._opts.center[0]
            let latitude = this.circleObj._opts.center.lat
              ? this.circleObj._opts.center.lat
    ...
    getList() {
      getDeliveryDcList(this.listQuery).then(response => {
        if (response.retCode === 200) {
          this.leftTextList = response.retData.pageData
          if (this.leftTextList.length) {
            ...
            this.leftTextList.forEach((v, i) => {
              if (v.verify) {
                this.getText(v.dcName, [v.longitude * 1, v.latitude])
